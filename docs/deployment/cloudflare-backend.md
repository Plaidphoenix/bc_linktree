...
- Converts `usr_admin` to the approved initial Access administrator referenced by `PRIVATE-EVIDENCE-02`; the public migration retains a non-routable placeholder and fails closed until private provisioning.
...
3. Authenticate as the approved administrator from `PRIVATE-EVIDENCE-02` with the one-time code sent by Cloudflare.
